    ...
    public Boolean check(AgentsDAO agentsDAO, MetricsDAO metricsDAO, Agent agent, Metrics metrics) {
        ...
        try {
            ...
            if (security.length() > 0) {
                Logger.getLogger(HomepageChecker.class.getName()).log(Level.INFO, String.format("\n-----> %1$s %2$s ssl error: %3$s", agent.id.toString(), homepage, security.toString()));
            }
