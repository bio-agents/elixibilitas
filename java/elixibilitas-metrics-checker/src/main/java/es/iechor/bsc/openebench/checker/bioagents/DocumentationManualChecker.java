...
public class DocumentationManualChecker implements MetricsChecker {
    ...
    public Boolean check(AgentsDAO agentsDAO, MetricsDAO metricsDAO, Agent agent, Metrics metrics) {
        ...
        if (Boolean.TRUE.equals(bool)) {
            ...
            documentation.setManual(true);
        } else if (project != null && project.getDocumentation() != null) {
            project.getDocumentation().setManual(bool);
        }
        return bool;
    }
    ...
    private static Boolean check(Agent agent) {
        ...
        return documentation.getManual()== null ? null : true;
    }
}
