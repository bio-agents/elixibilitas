/**
 * *****************************************************************************
 * Copyright (C) 2019 IECHOR ES, Spanish National Bioinformatics Institute (INB)
 ...
 *****************************************************************************
 */
package es.iechor.bsc.openebench.query;
...
import es.iechor.bsc.elixibilitas.dao.AbstractDAO;
import static es.iechor.bsc.elixibilitas.dao.AbstractDAO.LICENSE;
...
public class MongoQueries {
    ...
    public static int searchAgentsCount(AgentsDAO agentsDAO, String id, String text, String name, String homepage, String description, List<String> tags) {
        
        try {
            final MongoCollection<Document> col = agentsDAO.database.getCollection(agentsDAO.collection);
            ...
            if (id != null && !id.isEmpty()) {
                aggregation.add(agentsDAO.createIdFilter(id));
            }

            aggregation.add(Aggregates.group(new BasicDBObject("_id", "$_id.id"), Accumulators.push("agents", "$$ROOT")));

            aggregation.add(Aggregates.sort(Sorts.ascending("agents.name")));

            aggregation.add(Aggregates.unwind("$agents"));
            aggregation.add(Aggregates.replaceRoot("$agents"));
    ...
    /**
     * Find agents and write them into the reader.
     * 
     * @param agentsDAO
     * @param writer writer to write metrics into.
     * @param id
     * @param skip mongodb skip parameter (aka from).
     * @param limit mongodb limit parameter (limits number of agents to be written).
     ...
     */
    public static void searchAgents(AgentsDAO agentsDAO, Writer writer, String id, Long skip, Long limit, 
            String text, String name, String homepage, String description, List<String> projections) {
        searchAgents(agentsDAO, writer, id, skip, limit, text, name, homepage, description, null, projections);
    }
            ...
            try (JsonWriter jwriter = new AbstractDAO.ReusableJsonWriter(writer)) {
                ...
                try (MongoCursor<Document> cursor = iterator.iterator()) {
                    while (cursor.hasNext()) {
                        ...
                        doc.append("@id", agentsDAO.getURI(_id));
                        doc.append("@type", agentsDAO.getType(_id));
                        doc.append("@label", agentsDAO.getLabel(_id));
                        doc.append("@version", agentsDAO.getVersion(_id));
    ...
    public static int aggregateAgentsCount(
            AgentsDAO agentsDAO, String id, String text, String name, 
            String description, List<String> types, String[] edam_terms) {
        return aggregateAgentsCount(agentsDAO, id, text, name, description, null, types, edam_terms);
    }
    ...
    public static void aggregateAgents(
            AgentsDAO agentsDAO, MetricsDAO metricsDAO,
        ...
        aggregateAgents(agentsDAO, metricsDAO, writer, id, skip, limit, text, name, description, null, types, projections, edam_terms);
    }
                ...
                try (MongoCursor<Document> cursor = iterator.iterator()) {
                    if (cursor.hasNext()) {
                        do {
                            ...
                            List<Document> agents = doc.get("entries", List.class);
                            for (Document agent : agents) {
                                Document _id = (Document) agent.remove("_id");
                                agent.append("@id", agentsDAO.getURI(_id));
                                agent.append("@type", agentsDAO.getType(_id));
                                agent.append("@label", agentsDAO.getLabel(_id));
                                agent.append("@version", agentsDAO.getVersion(_id));
                                agent.append("@license", LICENSE);
                                
                                final String type = agentsDAO.getType(_id);
                            ...
                            do {
                                ...
                                jwriter.writeStartArray("agents");
