...
public class AgentsDAO extends AbstractDAO<Document> implements Serializable {
    
    public final static String COLLECTION = "agents";
    
    public AgentsDAO(MongoDatabase database, String baseURI) {
        super(baseURI, database, COLLECTION);
    }

    /**
     * Create a primary key from the agent URI
     * 
     * @param id the agent identifier in a form "$nmsp:$id/$type/$authority"
     * @return the primary key for the agent
     */
    @Override
        ...
        try {
            ...
            try (MongoCursor<Document> cursor = iterator.iterator()) {
                while (cursor.hasNext()) {
                    agents.add(deserialize(cursor.next()));
                }
            }
        } catch(Exception ex) {
    ...
    private Agent deserialize(final Document doc) {
    ...
    public String getAgents(String id) {
        final List<Document> docs = getBSON(id);
        if (docs.isEmpty()) {
            return null;
        }
    ...
    public String put(String user, Agent agent) {
        try {
            final String json = jsonb.toJson(agent);
            return put(user, agent.id.toString().substring(baseURI.length()), json);
        } catch(Exception ex) {
            Logger.getLogger(AgentsDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;
    ...
    public String upsert(String user, Agent agent, String id) {
        try {
            final String json = jsonb.toJson(agent);
    ...
    /**
     * Write agents ids those semantic annotations found in the map.
     * 
